                ...
                <div class="callout border-0">
                    ...
                    <table class="table table-hover ">
                        ...
                        while($row=$lt->fetch_assoc()):
                            $used = $conn->query("SELECT SUM(`leave_days`) as total FROM `leave_applications` where user_id = '{$id}' and status = 4 and date_format(date_start,'%Y') = '".date('Y')."' and date_format(date_end,'%Y') = '".date('Y')."' and leave_type_id = '{$row['id']}' ")->fetch_array()['total'];
            ...
            <div class="col-md-8 col-sm-12">
                <div class="callout border-0">
                    ...
                    <table class="table-stripped table">
                        ...
                        <tbody>
                            <?php 
                            $la = $conn->query("SELECT l.*,lt.code, lt.name FROM `leave_applications` l inner join `leave_types` lt on l.leave_type_id = lt.id where l.status = 4 and l.user_id = '{$id}' and (date_format(l.date_start,'%Y') = '".date("Y")."' or date_format(l.date_end,'%Y') = '".date("Y")."')  order by unix_timestamp(l.date_start) asc,unix_timestamp(l.date_end) asc ");
